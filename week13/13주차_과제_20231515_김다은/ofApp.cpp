...
bool ofApp::BFS() {
	...
	// bfs_path 초기화
	bfs_path.clear();
	vector<tuple<int, int, short>>().swap(bfs_path);
	...
	while (!q.empty()) {
		...
		for (int k = 0; k < maxdir; k++) {
			nx = x + dx[k];
			ny = y + dy[k];
			if ((!(0 <= nx && nx < inputmaze.size() && 0 <= ny && ny < inputmaze[0].size()))
				|| (inputmaze[nx][ny] == '|' || inputmaze[nx][ny] == '-')) {
				continue;
			}
			else {
				nx += dx[k]; ny += dy[k];
	...
	//	printf("bfs_path : %d %d %d\n", get<0>(bfs_path[i]), get<1>(bfs_path[i]), get<2>(bfs_path[i]));
	//}
	if (success) return true;
	return false;
}
...
void ofApp::bfsdraw() { 
	...
	int ratio = 20; // 미로 정답 선의 길이 비율
	int prex, prey, x, y, dir;

	// 탐색한 경로 그리기
	ofSetColor(0, 256, 0);
	for (int i = 1; i < bfs_path.size(); i++) {
		x = get<0>(bfs_path[i]); y = get<1>(bfs_path[i]); dir = get<2>(bfs_path[i]);
		prex = x - dx[dir]*2;  prey = y - dy[dir]*2;
		// printf("draw bfspath: %d %d %d %d\n", prex, prey, x, y);
		ofDrawLine(ratio * prey, ratio * prex, ratio * y, ratio * x);
	}

	// 정답 경로 그리기
	ofSetColor(256, 0, 0);
	x = get<0>(bfs_path[bfs_path.size() - 1]);
	y = get<1>(bfs_path[bfs_path.size() - 1]);
	dir = get<2>(bfs_path[bfs_path.size() - 1]);
	bool islast = 0;
	while (!islast) {
		prex = x - dx[dir]*2;
		prey = y - dy[dir]*2;
		for (int i = 0; i < bfs_path.size(); i++) {
			if (prex == get<0>(bfs_path[i]) && prey == get<1>(bfs_path[i])) {
				// printf("%d %d to %d %d\n", x, y, prex, prey);
				ofDrawLine(ratio * prey, ratio * prex, ratio * y, ratio * x);
				x = prex; y = prey; dir = get<2>(bfs_path[i]);
				if (i == 0) islast = 1;
				break;
			}
		}
